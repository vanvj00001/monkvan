...
find . -maxdepth 1 -not -name '.git' -not -name 'public' -not -name 'hugo.toml' -not -name 'deploy.sh' -not -name 'build-both-themes.sh' -not -name 'go.mod' -not -name 'go.sum' -not -name '.gitignore' -not -name '.gitmodules' -not -name 'README.md' -not -name 'LICENSE' -type f -exec rm -f {} +
